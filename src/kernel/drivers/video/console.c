...
{
	writeChars(value, consoleColor);
}

void console_write_char(char value) 
{
	char string[] = { value, 0 };
	writeChars(string, consoleColor);
}

void console_write_bool(bool value) 
{
	writeChars(value ? "True" : "False", consoleColor);
}
...
void console_write_int8(int8_t value) 
{
	...
	buffer += (sizeof(array) / sizeof(char)) - 1;
	*buffer = '\0';
	...
	writeChars(buffer, consoleColor);
}
